...
form.addEventListener('submit', function (e) {
     // automatically submit button send it to the server, so we prevent it
     e.preventDefault();
     ...
     if (isNaN(value) || value <= 0 || value > 9) {
          ...
          let myPara = text.slice(0, value)
          myPara = myPara.map(item => `<p class="result">${item}</p>`);
          result.innerHTML = myPara.join('');
